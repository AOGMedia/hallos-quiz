  ...
  <div className="bg-card border border-border rounded-xl overflow-hidden">
    ...
    {rankings.length === 0 ? (
      <div className="py-12 flex flex-col items-center justify-center text-center px-4">
        <Trophy className="w-10 h-10 text-yellow-500/30 mb-3" />
        <p className="text-sm font-medium text-muted-foreground">No tournament rankings yet</p>
        <p className="text-xs text-muted-foreground/70 mt-1">Join a tournament to carve your name here!</p>
      </div>
    ) : (
        ...
        >
          ...
          <div className="flex items-center justify-end gap-1">
            ...
            <span className="text-xs sm:text-sm font-semibold text-foreground">
              {entry.totalPrizeMoney.toLocaleString()}
            </span>
          </div>
